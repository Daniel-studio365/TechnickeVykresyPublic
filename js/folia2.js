...
function draw(){
  ...
  if(rollBounds){
    ...
    const headerW = 220;
    const margin = 10;
    const preBB = allGroup.getBBox();
    const headerY = preBB.y - headerH - margin;
...
function exportPDF(){
  draw();
  const liveGroup = svgRoot.querySelector('g.content-core') || svgRoot.querySelector('g.content-bbox') || svgRoot;
  const bb = liveGroup.getBBox();
  const clone = svgRoot.cloneNode(true);
  // remove navin + headers/footers for PDF export (only technical drawing)
  clone.querySelectorAll('.roll, .roll-group, .header-ui, .footer-ui').forEach(n=> n.remove());
