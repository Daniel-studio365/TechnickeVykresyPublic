...
(function(){
  ...
  if (btnOpenFirmManager) {
    btnOpenFirmManager.addEventListener('click', () => {
      ...
      window.open('../../shared/entry/index2.html?vz=vz34', '_blank');
    });
  }
  ...
  function prefillFromFirm(){
    ...
    const resolveAsset = (p)=>{
      if(!p) return null;
      if(p === 'zhora') return '../../shared/assets/zhora.png';
      if(p === 'zdola') return '../../shared/assets/zdola.png';
      return p;
    };
  ...
  function applyClipPreset(preset, fromUser=false){
    ...
    if (p === 'zhora') { setBottomImage('../../shared/assets/zhora.png'); return; }
    if (p === 'zdola') { setBottomImage('../../shared/assets/zdola.png'); return; }
  ...
  function draw(){
    ...
    if ($('PhotoMarkEnabled')?.checked){
      ...
      const photoY = yBottomBody - photoOffsetBottom - photoH;
      const photoColor = '#0f172a';
      const photoDimY = yBottomBody + Math.max(36, Math.round(state.fontPx*3.0));
      ...
      vDim(leftOuter - 42, photoY + photoH, yBottomBody, photoOffsetBottom, 10, photoColor);
      textWithBg('20x5', photoX + photoW/2, photoY - 18, {anchor:'middle', baseline:'middle', color:photoColor});
      minX = Math.min(minX, leftOuter - 70);
    }
    if(!state.lineOnly){
