...
(function(){
  ...
  if (btnOpenFirmManager) {
    btnOpenFirmManager.addEventListener('click', () => {
      ...
      window.open('../../shared/entry/index2.html?vz=vz22', '_blank');
    });
  }
  ...
  function prefillFromFirm(){
    ...
    const dataVz = data && data.vz === 'vz22_test' ? 'vz22' : data?.vz;
    if (!data || dataVz !== 'vz22') return;
    ...
    r.onload = (ev)=>{
      try{
        const data = JSON.parse(ev.target.result);
          if (data.vz) {
            data.vz = data.vz === 'vz22_test' ? 'vz22' : data.vz;
          }
